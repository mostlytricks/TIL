...
<details>
  <summary> 동시성은.. 하드웨어부터 고려하자 </summary>

  - 이번장의 핵심은, ~ 처럼이 아니었을까. 병렬처리가 가능한 하드웨어준비가 안되어 있다면, 사실 그럴듯하게 보이는 동작을 할 뿐이다.

  - 트랜잭션으로 관리해야하는 데이터를 동시적으로 접근해야 하는지 고민이 필요할 듯하다. 물론 RDB - 조회용 분산 DB로 데이터 카피를 한 뒤, 동시적으로 접근해야 하는 데이터는 분산 DB에서 정합성을 포기하고 조회하는 식이 맞겠지만... 아니면 구독을해서 변경시마다 알람을 줘야하나?
  
  - 동시성은 결국 데이터 전송 속도 까지 고려하면, 정보가 정합할 수 있는가? 라는 추상적인 명제에 닿는 듯하다. 정보가 틀릴 수 있다는 걸 전제로 확인하는 로직을 클라이언트 측에서 고도화 하는게 향후 방향일지도.
...
<details>
  <summary> 이 정도 내용은 팀을 위해 요약본을 출력해서 나눠줄만 하다 </summary>

- 무언가 이게 필요하지 않나? 싶은 대목들이 모두 읊어지는 기적을 보았다. 이거구나, 찾고 있던게

- 프로토타이핑, 의도에 따른 프로그래밍, 테스트, 용어 및 문화, 페어 프로그래밍. 기억하자.
...
<details>
  <summary> 이 정도 내용은 팀을 위해 요약본을 출력해서 나눠줄만 하다 - (2). 팀에 확산좀 해봐야겠다. </summary>

  - 페어 프로그래밍을 처음 경험한 것은 학부 4학년 때 였던 것 같다. 특정 과제 중 dfs구현 부분을 진행하면서였는데, 사실 부족한 구현 역량을 다른 사람이 커버해준 느낌이었다. 내가 무슨 말 하는지도 아마 잘 몰랐던 것 같고.

  - 최근 구현이 방식보다도, 소통의 방식에 대해 고민이 많다. 대뜸 만드세요, 보다 상대방이 어느 정도 나랑 생각이 일치하는지 조율하는 과정이 필요했는데. 페어 프로그래밍은 정기적인 세션으로, 리팩토링 대상 코드를 놓고서 하면 코드를 바라보는 시각, 네이밍에 대한 직관이 공유될 것 같다. 바로 적용해봐야지.
...
<details>
  <summary> 알잘딱깔센은 한쪽구석에 고이접어 넣어두고, 일단 질문을 하긴 해야한다.  </summary>

  - 요구사항을 사실 알아서 잘 깔끔하고 센스있게 정리해서 주면 결과물이 좋아질텐데, 밈의 남용은 사회적 패악이다.
...
<details>
  <summary> 자기 작업물에 책임질 수 있어야 한다. 그것이..프로니까.  </summary>

- 그렇다. 프로의 길은 작업물에 대한 책임이었다. 아니, 자부심이 있어야 하는 것이었다.

- 거장이 자기 그림을 찢고 다시 그리는 괴짜들이 있다 하더라도, 이 그림은 다른 사람이 그렸어요 하지는 않지 않던가. 내가 그렸기에 아쉬우면 찢고, 아쉽지 않으면 내 그림이라고 덤덤히 한 마디 하는 거지. 분발해야겠다.
